!function() {
    ...
    var loop = null, track = null, powerupID = 0, resizeCanvas = {
        get fullscreen() {
    ...
    class Item {
        ...
        collide(t) {
            if(t.pos.distanceToSquared(this.pos) < 500 && !t.track.tb) {
                this.activate(t);
            }
        }
        erase(t) {
            if(t.distanceTo(this.pos) < ab + 7) {
                this.remove();
                return this
            }
            return false;
    ...
    class Bomb extends Item {
        ...
        type = "O";
        activate(a){
            this.track.firstPlayer = this.track.players[0] = new Explosion(this.pos, a.track.gravity, a.track.time, this.track, this.track.firstPlayer.checkpoints)
        }
    }
    ...
    class Track {
        constructor(a){
            ...
            for(c = e.length; a < c; a++)
                ...
                2 < h.length){
                    ...
                    this.grid[b][d] === void 0 && (this.grid[b][d] = new Sector),
        ...
        draw(){
            ...
            if(track.cameraLock && !Hb && ("line" === tool || "scenery line" === tool || "brush" === tool || "scenery brush" === tool || "teleporter" === tool))
                ...
                for(y = i.y; y <= l.y; y++)
                    if(h[w] !== void 0 && h[w][y] !== void 0){
                        if(0 < h[w][y].physics.length || 0 < h[w][y].scenery.length){
                            ...
                            K.drawImage(this.sectors[C], Math.floor(canvas.width / 2 - this.camera.x * d + w * e * d), Math.floor(canvas.height / 2 - this.camera.y * d + y * e * d))
                        }
                        K.strokeStyle = "#000";
                        n = 0;
            ...
            if(250 !== canvas.width){
                if(Hb)
                    ...
                    case "antigravity":
                    case "teleporter":
                        K.fillStyle = tool == "goal" ? "#ff0" : tool == "checkpoint" ? "#00f" : tool == "bomb" ? "#f00" : tool == "slow-mo" ? "#eee" : tool == "antigravity" ? "#0ff" : "#f0f";
                        K.beginPath(),
                        K.arc(f.x, f.y, 7 * d, 0, 2 * Math.PI, !0),
                        K.fill(),
                        K.stroke();
                        break;
                    case "boost":
                    case "gravity":
                        K.beginPath(),
                        K.fillStyle = tool == "boost" ? "#ff0" : "#0f0",
                        K.save();
                        if(track.cameraLock) {
                            K.translate(U.toPixel().x, U.toPixel().y),
                            K.rotate(Math.atan2(-(R.x - U.x), R.y - U.y));
                        } else {
                            K.translate(f.x, f.y);
                        }
                        K.moveTo(-7 * d, -10 * d),
    ...
    class SurvivalTrack extends Track {
        constructor() {
            super("SURVIVAL");
        ...
        update() {
            ...
            var a;
            a = (a = this.physics[this.physics.length - 1]) ? a.b : new Vector(-50,50);
        ...
        draw() {
            ...
            K.clearRect(0, 0, canvas.width, canvas.height);
            var b = (new Vector(0,0)).adjustToCanvas()
            , c = (new Vector(canvas.width,canvas.height)).adjustToCanvas();
                ...
                for(i = b.y; i <= c.y; i++)
                    if(this.grid[h] !== void 0 && this.grid[h][i] !== void 0){
                        if(0 < this.grid[h][i].physics.length){
                            d[h + "_" + i] = 1;
                            if(this.sectors[h + "_" + i] === void 0){
                                e = this.sectors[h + "_" + i] = document.createElement("canvas");
                                ...
                                for(f = this.grid[h][i].physics.length; e < f; e++)
                                    this.grid[h][i].physics[e].draw(this.sectors[h + "_" + i].getContext("2d"), h * this.scale * this.zoom, i * this.scale * this.zoom)
                            }
                            K.drawImage(this.sectors[h + "_" + i], Math.floor(canvas.width / 2 - this.camera.x * this.zoom + h * this.scale * this.zoom), Math.floor(canvas.height / 2 - this.camera.y * this.zoom + i * this.scale * this.zoom))
                        }
                        K.strokeStyle = "#000"
                    }
            for(var l in this.sectors)
                d[l] === void 0 && delete this.sectors[l];
    ...
    class Ride {
        constructor(a, b) {
            track = this.track = a == "SURVIVAL" ? new SurvivalTrack(a) : new Track(a);
            window.Game.track = () => this.track;
            resizeCanvas.smallscreen;
            this.track.sb = b || [];
            ...
            this.update = [];
            this.render = [];
            this.update.push((a) => this.track.update(a));
            this.render.push((a) => this.track.render(a));
    ...
    document.onkeydown = function(a){
        ...
        if(location.pathname.slice(0, 7) == '/tracks')
            ...
            case 83:
                track.firstPlayer && (a.preventDefault(),
                track.cameraFocus = track.firstPlayer.head,
                track.firstPlayer.alive && track.firstPlayer.setButtonDown("down"),
                track.firstPlayer.checkpointsCache = []);
                break;
            }
        if(track.id == void 0)
            ...
            case 81:
                "line" !== tool ? (tool = "line",
                document.body.style.cursor = "none") : track.cameraLock || (track.cameraLock = !0,
                U.copy(Lb),
                Z = !0);
                break;
            case 87:
    ...
    document.onkeyup = function(a){
        ...
        case 90:
            ...
            break;
        case 71:
            track.players.length > 1 ? track.cameraFocus = track.players[1].head === track.cameraFocus && track.firstPlayer ? track.firstPlayer.head : track.players[1].head : (Jb = 11 - Jb,
        ...
        if(location.pathname.slice(0, 7) == '/tracks')
            ...
            case 87:
                ...
                break;
            case 83:
                track.firstPlayer.gamepad.down = 0;
                break;
            }
    },
    ...
    document.onmousemove = function(a){
        "camera" !== tool && (track.cameraFocus = !1);
